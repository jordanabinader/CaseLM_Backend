...
class CaseDiscussionWorkflow:
    ...
    def setup_nodes(self):
        ...
        self.workflow.add_node(
            "direct_human_response_agent", self.direct_human_response
        )
    ...
    def setup_edges(self):
        ...
        self.workflow.add_conditional_edges(
            ...
            {"create_topics": "create_topics", END: END},
        )
        self.workflow.add_conditional_edges(
            "create_topics",
            self.topic_creation_condition,
            {"create_plan": "create_plan", END: END},
        )
        self.workflow.add_conditional_edges(
            "create_plan",
            self.planning_condition,
            {"assign_discussion": "assign_discussion", END: END},
        )
        self.workflow.add_conditional_edges(
            "assign_discussion",
            self.assignment_condition,
            {"execute_discussion": "execute_discussion", END: END},
        )
    ...
    async def create_personas(self, state: DiscussionState) -> Dict[str, Any]:
        ...
        self.professor_introduction_statement = result["professor"][
        ...
        return {"personas": result["personas"], "current_step": "create_personas"}
    ...
    async def create_topics(self, state: DiscussionState) -> Dict[str, Any]:
        ...
        return {"topics": result["plan"], "current_step": "create_topics"}
    ...
    async def create_plan(self, state: DiscussionState) -> Dict[str, Any]:
        ...
        return {"discussion_plan": result["plan"], "current_step": "create_plan"}
    ...
    async def execute_discussion(self, state: DiscussionState) -> Dict[str, Any]:
        ...
        formatted_message = {
            ...
            "uuid": state["personas"]
            .get(result["discussion"]["response"]["speaker"], {})
            .get("uuid")
            or result["discussion"]["response"].get("uuid"),
            "references": result["discussion"]["response"].get(
    ...
    async def evaluate_discussion(self, state: DiscussionState) -> Dict[str, Any]:
        ...
        evaluation_message = {
            ...
            "suggested_next_speaker": result["evaluation"].get(
                "suggested_next_speaker", ""
            ),
            "sequence_complete": result["evaluation"].get("sequence_complete", False),
            "current_topic_complete": result["evaluation"].get(
        ...
        return {
            "evaluations": [
                evaluation_message
            ],  # Override previous evaluations instead of appending
    ...
    async def summarize_discussion(self, state: DiscussionState) -> Dict[str, Any]:
        ...
        return {"summaries": [{"role": "assistant", "content": str(result["summary"])}]}
    ...
    async def orchestrate(self, state: DiscussionState) -> Dict[str, Any]:
        ...
        return {
            "complete": result["next_step"] == "complete",
    ...
    async def direct_human_response(self, state: DiscussionState) -> Dict[str, Any]:
        ...
        result = await self.direct_human_response_agent.process(
            {"user_inputs": state["user_inputs"], "case_content": state["case_content"]}
        )
        from src.main import create_message as db_create_message
    ...
    async def handle_user_input(self, state: DiscussionState) -> Dict[str, Any]:
        ...
        return {
            "awaiting_user_input": True,
            "messages": [
                {"role": "system", "content": "Still waiting for your response..."}
            ],
        }
    ...
    async def assign_discussion(self, state: DiscussionState) -> Dict[str, Any]:
        ...
        current_sequence = (
            state.get("current_sequence") or state["discussion_plan"]["sequences"][0]
        )
        ...
        print(f"result: {result}")
        assignment_message = {"role": "system", "content": str(result["assignment"])}

        return {"assignments": [assignment_message]}
    ...
    async def replan_sequence(self, state: DiscussionState) -> Dict[str, Any]:
        ...
        if hasattr(latest_evaluation, "additional_kwargs"):
            suggested_next_speaker = latest_evaluation.additional_kwargs.get(
                "suggested_next_speaker"
            )
        else:
        ...
        return {
            "discussion_plan": result[
                "updated_plan"
            ],  # This will update the main state's discussion_plan
            "current_sequence": result["updated_plan"]["sequences"][
                0
            ],  # Pass the current sequence explicitly
    ...
    def evaluation_condition(self, state: DiscussionState) -> str:
        ...
        sequence_complete = latest_evaluation.additional_kwargs.get(
            "sequence_complete", False
        )
        current_topic_complete = latest_evaluation.additional_kwargs.get(
            "current_topic_complete", False
        )
    ...
    async def run(self, case_content: str, *, human_participant: Dict[str, Any]):
        ...
        try:
            async for state in self.graph.astream(
                self.current_state, config={"recursion_limit": 1000}
            ):
                ...
                if state.get("messages"):
                    for message in state["messages"]:
                        ...
                        if isinstance(message, dict):
                            ...
                            speaker = (
                                message.additional_kwargs.get("speaker", "System")
